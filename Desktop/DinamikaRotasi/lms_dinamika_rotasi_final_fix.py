# lms_dinamika_rotasi_final_fix.py
    ...
    if not os.path.exists(file_path):
        if file_path.endswith(".csv"):
            ...
            if key == "video_apresiasi":
                ...
                # 🔥 PERBAIKAN DI SINI: Struktur lengkap untuk Pertemuan 1 & 2
                default_data = {
...
# 🔥 TAMBAHKAN FUNGSI check_hadir() DI SINI
def check_hadir():
...
def guru_page():
    ...
    # --- MENU: Pre-test ---
        ...
        st.subheader("🧠 Kelola Soal Pre-test")
        ...
        # Form edit soal
        with st.form("form_edit_pre_test"):
            judul_baru = st.text_input("Judul Pre-test", value=pre_test_data.get("judul", ""))
            desc_baru = st.text_area("Deskripsi Pre-test", value=pre_test_data.get("deskripsi", ""), height=150)
            
            soal_list = pre_test_data.get("soal_list", [])
            soal_baru_list = []
            for i, soal in enumerate(soal_list):
                teks_baru = st.text_area(f"Soal {i+1}", value=soal.get("teks", ""), key=f"pre_soal_{i}")
                soal_baru_list.append({"id": soal.get("id", f"pre_q{i+1}"), "teks": teks_baru})

            submitted = st.form_submit_button("💾 Simpan Soal Pre-test")
        ...
        if submitted:
            ...
            simpan_data("pre_test", data_baru)
            st.success("✅ Soal pre-test berhasil disimpan!")

        # Tampilkan pratinjau
        st.divider()
        st.subheader("👁️‍🗨️ Pratinjau Pre-test untuk Siswa")
    ...
    # --- MENU: Media Pembelajaran ---
        ...
        st.subheader("📚 Upload Media Pembelajaran")
        media_data = muat_data("media_pembelajaran")
        if not media_data:
            st.error("Media pembelajaran belum diatur oleh guru.")
            return

        # 🔧 PERBAIKAN STRUKTUR DATA SECARA DINAMIS
        # Pastikan struktur data selalu lengkap
        if "pertemuan_1" not in media_data:
            media_data["pertemuan_1"] = {
                "judul": "Pertemuan 1",
                "file_bahan_ajar": "",
                "deskripsi": "Bahan ajar untuk Pertemuan 1",
                "videos": [],
        ...
        if "pertemuan_2" not in media_data:
            media_data["pertemuan_2"] = {
                "judul": "Pertemuan 2",
                "file_bahan_ajar": "",
                "deskripsi": "Bahan ajar untuk Pertemuan 2",
                "videos": [],
        ...
        with tab1:
            ...
            # Tampilkan daftar video & gambar
            st.subheader("🎬 Daftar Video Pertemuan 1")
            for i, url in enumerate(media_data["pertemuan_1"]["videos"]):
                st.markdown(f"{i+1}. {url}")
                st.video(url)
            st.subheader("🖼️ Daftar Gambar Pertemuan 1")
            for i, url in enumerate(media_data["pertemuan_1"]["images"]):
                st.markdown(f"{i+1}. {url}")
        ...
        with tab2:
            ...
            # Tampilkan daftar video & gambar
            st.subheader("🎬 Daftar Video Pertemuan 2")
            for i, url in enumerate(media_data["pertemuan_2"]["videos"]):
                st.markdown(f"{i+1}. {url}")
                st.video(url)
            st.subheader("🖼️ Daftar Gambar Pertemuan 2")
            for i, url in enumerate(media_data["pertemuan_2"]["images"]):
                st.markdown(f"{i+1}. {url}")
                st.image(url, caption=f"Gambar {i+1}", use_column_width=True)
    ...
    # --- MENU: Simulasi Virtual ---
        ...
        with st.form("form_edit_simulasi"):
            ...
            for i, sim in enumerate(simulasi_list):
                judul_sim_baru = st.text_input(f"Judul Simulasi {i+1}", value=sim.get("judul", ""), key=f"sim_judul_{i}")
                url_sim_baru = st.text_input(f"URL Simulasi {i+1}", value=sim.get("url", ""), key=f"sim_url_{i}")
...
def siswa_page():
    ...
    # --- MENU: LKPD ---
        ...
        if st.button("✅ Kirim Jawaban LKPD"):
            if any(not v.strip() for v in jawaban_analisis.values()) or not kesimpulan.strip():
                ...
                df = pd.concat([df, new_entry], ignore_index=True)
                simpan_data("hasil_penilaian", df)
                st.success("✅ Jawaban LKPD berhasil dikirim!")
