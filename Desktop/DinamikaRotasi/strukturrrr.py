    ...
    if menu == "Daftar Hadir":
        ...
        with tab1:
            ...
            if st.button(f"🗑️ Hapus Video {i+1}", key=f"hapus_video_p2_{i}"):
                ...
                media_data["pertemuan_1"] = p2_data
                media_data["waktu_update"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        ...
        with tab2:
            ...
            if st.button(f"🗑️ Hapus Gambar {i+1}", key=f"hapus_gambar_p2_{i}"):
                ...
                with st.form("form_edit_simulasi"):
                      ...
                      # Muat data simulasi list
                     simulasi_list = simulasi_data.get("simulasi_list", [])
            
                    # Form edit simulasi (hanya jika ada simulasi)
                    if simulasi_list:
                            # Hanya ambil simulasi pertama (Balancing Act)
                            simulasi = simulasi_list[0]
               ...
               # Tombol Simpan
               submitted = st.form_submit_button("💾 Simpan Seluruh Perubahan Simulasi Virtual")
