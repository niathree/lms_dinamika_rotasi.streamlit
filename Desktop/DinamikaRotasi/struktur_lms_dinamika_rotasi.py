...
FILES = {
    ...
    "pre_test_info": "pre_test_info.json",
    ...
    if not os.path.exists(file_path):
        if file_path.endswith(".csv"):
            if key == "data_hadir":
                ...
                pd.DataFrame(columns=["email", "nama", "jenis_penilaian", "nilai", "jawaban_json", "waktu_kerja", "role"]).to_csv(file_path, index=False)
...
def muat_data(file_key):
    ...
    if os.path.exists(file_path):
        ...
        except (json.JSONDecodeError, FileNotFoundError, pd.errors.EmptyDataError):
            pass
    return None
...
def guru_page():
    ...
    if menu == "Daftar Hadir":
        ...
        pre_test_data = muat_data("pre_test_info")
        ...
        if submitted:
            ...
            simpan_data("pre_test_info", data_baru)
        ...
        with tab1:
            ...
            bahan_ajar_p1 = st.text_area("📝 Bahan Ajar (Ditulis Langsung)", value=p1_data.get("bahan_ajar", ""), height=300)
            
            # Upload video & gambar tambahan
            st.subheader("🖼️ Upload Video & Gambar Tambahan")
            ...
            if st.button("➕ Tambahkan Video/Gambar", key="tambah_p1"):
                if new_video_url:
                    if "youtube.com/embed/" in new_video_url:
                        ...
                        st.error("Harap gunakan URL 'embed' YouTube.")
                if new_image_url:
            ...
            # Tombol Simpan
            if st.button("💾 Simpan Media Pertemuan 1"):
                ...
                simpan_data("media_pembelajaran", media_data)
                st.success("✅ Media Pertemuan 1 berhasil disimpan!")
        ...
        with tab2:
            ...
            # Bahan Ajar (ditulis langsung)
            bahan_ajar_p2 = st.text_area("📝 Bahan Ajar (Ditulis Langsung)", value=p2_data.get("bahan_ajar", ""), height=300)
            ...
            if st.button("➕ Tambahkan Video/Gambar", key="tambah_p2"):
                if new_video_url:
                    if "youtube.com/embed/" in new_video_url:
            ...
            # Tombol Simpan
            if st.button("💾 Simpan Media Pertemuan 2"):
                ...
                media_data["waktu_update"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                simpan_data("media_pembelajaran", media_data)
                st.success("✅ Media Pertemuan 2 berhasil disimpan!")
        ...
        with st.form("form_edit_simulasi"):
            judul_baru = st.text_input("Judul Simulasi", value=simulasi_data.get("judul", ""))
            desc_baru = st.text_area("Deskripsi Simulasi", value=simulasi_data.get("deskripsi", ""), height=100)
            petunjuk_baru = st.text_area("Petunjuk Penggunaan", value=simulasi_data.get("petunjuk_penggunaan", ""), height=200)
        ...
        # Tampilkan pratinjau
        st.divider()
        st.subheader("👁️‍🗨️ Pratinjau Simulasi untuk Siswa")
        ...
        for i, sim in enumerate(simulasi_data.get("simulasi_list", []), 1):
            ...
            # st.components.v1.iframe(sim["url"], height=600, scrolling=True) # PhET mungkin tidak bisa diembed
        ...
        with st.form("form_edit_post_test"):
            ...
            soal_baru_list = []
            for i in range(10): # Misalnya 10 soal
                if i < len(soal_list):
                    soal = soal_list[i]
                else:
                    soal = {"id": f"pt{i+1}", "teks": "", "opsi": ["", "", "", "", ""], "kunci": "A", "skor": 10}
...
def siswa_page():
    ...
    if menu == "Daftar Hadir":
        ...
        # Pilih Pertemuan
        pertemuan = st.selectbox("Pilih Pertemuan:", ["Pertemuan 1", "Pertemuan 2"], key="pilih_pertemuan_pretest")
        if pertemuan == "Pertemuan 1":
            soal_list = pre_test_data.get("soal_list", [])[:4] # Ambil 4 soal pertama
        else: # Pertemuan 2
            soal_list = pre_test_data.get("soal_list", [])[4:] # Ambil sisanya
