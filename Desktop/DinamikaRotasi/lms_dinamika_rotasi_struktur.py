# lms_dinamika_rotasi_versi_fix_akhir.py
...
LKPD_INFO_FILE = "lkpd_info.json"
POST_TEST_INFO_FILE = "post_test_info.json"
REFLEKSI_INFO_FILE = "refleksi_info.json"
HASIL_NILAI_FILE = "hasil_nilai.csv"
FORUM_FILE = "forum_diskusi.csv"

# Inisialisasi file jika belum ada
# Daftar Hadir
if not os.path.exists(DATA_HADIR_FILE):
    pd.DataFrame(columns=["email", "nama", "status", "waktu", "role"]).to_csv(DATA_HADIR_FILE, index=False)
...
if not os.path.exists(MEDIA_INFO_FILE):
    ...
    with open(MEDIA_INFO_FILE, "w") as f:
        json.dump(default_media, f)
...
if not os.path.exists(LKPD_INFO_FILE):
    ...
    with open(LKPD_INFO_FILE, "w") as f:
        json.dump(default_lkpd, f)
...
if not os.path.exists(REFLEKSI_INFO_FILE):
    ...
    with open(REFLEKSI_INFO_FILE, "w") as f:
        json.dump(default_refleksi, f)

# Hasil Nilai
if not os.path.exists(HASIL_NILAI_FILE):
    pd.DataFrame(columns=["email", "nama", "jenis_penilaian", "nilai", "waktu_kerja", "role"]).to_csv(HASIL_NILAI_FILE, index=False)

# Forum Diskusi
if not os.path.exists(FORUM_FILE):
    pd.DataFrame(columns=["id", "parent_id", "email", "nama", "pesan", "waktu", "role"]).to_csv(FORUM_FILE, index=False)
...
def muat_data(file_path):
    ...
    if os.path.exists(file_path):
        ...
        except (json.JSONDecodeError, pd.errors.EmptyDataError, FileNotFoundError):
            pass
    return None
...
# === HALAMAN GURU: DASBOR LENGKAP ===
def guru_page():
    st.header("👩‍🏫 Dasbor Guru: LMS Dinamika Rotasi")
    ...
    # --- MENU: Video Apersepsi ---
        ...
        st.subheader("🎥 Upload & Kelola Video Apersepsi")
        video_data = muat_data(VIDEO_INFO_FILE)
        if not video_data:
            st.error("File video info rusak.")
            return

        # Form upload video & edit deskripsi
        ...
        if submitted:
            ...
            simpan_data(VIDEO_INFO_FILE, video_data)
    ...
    # --- MENU: Pre-test ---
        ...
        st.subheader("🧠 Kelola Pre-test")
        st.info("Pre-test akan ditampilkan di halaman siswa. Anda bisa mengeditnya di kode.")

    # --- MENU: Deskripsi Materi ---
    elif menu == "Deskripsi Materi":
        st.subheader("📚 Edit Deskripsi Materi")
        st.info("Deskripsi materi akan ditampilkan di halaman siswa. Anda bisa mengeditnya di kode.")

    # --- MENU: Media Pembelajaran ---
    elif menu == "Media Pembelajaran":
        st.subheader("📚 Upload Media Pembelajaran (Bahan Ajar)")
        media_data = muat_data(MEDIA_INFO_FILE)
        if not media_data:
            st.error("File media info rusak.")
            return
        ...
        with tab1:
            ...
            if st.button("💾 Simpan Deskripsi Pertemuan 1"):
                ...
                simpan_data(MEDIA_INFO_FILE, media_data)
        ...
        tab1_pratinjau, tab2_pratinjau = st.tabs(["Pertemuan 1", "Pertemuan 2"])

        with tab1_pratinjau:
            st.write(f"**{media_data['pertemuan_1'].get('judul', 'Pertemuan 1')}")
            st.info(media_data["pertemuan_1"].get("deskripsi", ""))
            if media_data["pertemuan_1"].get("file_bahan_ajar"):
                pdf_path = os.path.join(UPLOAD_FOLDER, media_data["pertemuan_1"]["file_bahan_ajar"])
                if os.path.exists(pdf_path):
                    with open(pdf_path, "rb") as f:
                        st.download_button("📄 Unduh Bahan Ajar Pertemuan 1", f, media_data["pertemuan_1"]["file_bahan_ajar"], "application/pdf")
                else:
                    st.warning("📁 File PDF Pertemuan 1 tidak ditemukan.")

        with tab2_pratinjau:
            st.write(f"**{media_data['pertemuan_2'].get('judul', 'Pertemuan 2')}")
            st.info(media_data["pertemuan_2"].get("deskripsi", ""))
            if media_data["pertemuan_2"].get("file_bahan_ajar"):
                pdf_path = os.path.join(UPLOAD_FOLDER, media_data["pertemuan_2"]["file_bahan_ajar"])
                if os.path.exists(pdf_path):
                    with open(pdf_path, "rb") as f:
                        st.download_button("📄 Unduh Bahan Ajar Pertemuan 2", f, media_data["pertemuan_2"]["file_bahan_ajar"], "application/pdf")
                else:
                    st.warning("📁 File PDF Pertemuan 2 tidak ditemukan.")

    # --- MENU: Simulasi Virtual ---
    elif menu == "Simulasi Virtual":
        st.subheader("🧪 Kelola Simulasi Virtual")
        st.info("Simulasi virtual akan ditampilkan di halaman siswa. Anda bisa mengeditnya di kode.")

    # --- MENU: LKPD ---
    elif menu == "LKPD":
        st.subheader("📄 Edit LKPD")
        lkpd_data = muat_data(LKPD_INFO_FILE)
        if not lkpd_data:
            st.error("File LKPD info rusak.")
            return
        ...
        if submitted:
            ...
            simpan_data(LKPD_INFO_FILE, data_baru)
            st.success("✅ Seluruh LKPD berhasil diperbarui!")

    # --- MENU: Refleksi Siswa ---
    elif menu == "Refleksi Siswa":
        st.subheader("💭 Edit Refleksi Siswa")
        refleksi_data = muat_data(REFLEKSI_INFO_FILE)
        if not refleksi_data:
            st.error("File refleksi info rusak.")
            return
        ...
        if submitted:
            ...
            simpan_data(REFLEKSI_INFO_FILE, data_baru)
            st.success("✅ Refleksi siswa berhasil diperbarui!")

    # --- MENU: Post-test ---
    elif menu == "Post-test":
        st.subheader("📝 Edit Soal Post-test")
        post_test_data = muat_data(POST_TEST_INFO_FILE)
        if not post_test_data:
            st.error("File post-test info rusak.")
            return
        ...
        with st.form("form_edit_post_test"):
            ...
            for i, soal in enumerate(soal_list):
                ...
                skor_baru = st.number_input("Skor Soal", min_value=1, value=soal.get("skor", 1), key=f"pt_skor_{i}")
        ...
        if submitted:
            ...
            simpan_data(POST_TEST_INFO_FILE, data_baru)
    ...
    # --- MENU: Hasil Penilaian ---
        ...
        st.subheader("📊 Hasil Penilaian Siswa")
        df = muat_data(HASIL_NILAI_FILE)
        if df is not None and not df.empty:
            df_siswa = df[df["role"] == "siswa"]
            st.dataframe(df_siswa[["nama", "email", "jenis_penilaian", "nilai", "waktu_kerja"]].sort_values(by="waktu_kerja", ascending=False))
        else:
            st.info("Belum ada data penilaian.")

# === HALAMAN SISWA: DASBOR LENGKAP ===
def siswa_page():
    ...
    if menu == "Daftar Hadir":
        ...
        if st.button("✅ Simpan Kehadiran"):
            if not nama.strip():
                ...
                simpan_data(DATA_HADIR_FILE, df)
    ...
    # --- MENU: Pre-test ---
        ...
        check_hadir()
        st.info("Jawablah pertanyaan berikut sebelum mempelajari materi.")

        # Soal pre-test (hardcoded untuk saat ini)
        soal_pretest = [
        ...
        for i, soal in enumerate(soal_pretest):
            ...
            jawaban = st.text_area("Jawaban Anda:", key=f"pre_q{i+1}", height=100)
            jawaban_dict[f"pre_q{i+1}"] = jawaban
        ...
        if st.button("✅ Kirim Jawaban Pre-test"):
            if any(not v.strip() for v in jawaban_dict.values()):
                ...
                # Simpan jawaban (opsional, tidak dinilai)
                st.success("✅ Jawaban pre-test berhasil dikirim!")

    # --- MENU: Deskripsi Materi ---
    elif menu == "Deskripsi Materi":
        st.subheader("📚 Deskripsi Materi: Dinamika Rotasi")
        check_hadir()
        st.markdown("### 🎯 Capaian Pembelajaran (Fase F)")
        st.write("""
Pada fase F, peserta didik mampu menerapkan konsep dan prinsip vektor ke dalam kinematika dan dinamika gerak rotasi, 
usaha dan energi dalam sistem rotasi, serta dinamika fluida dalam gerak berputar. Peserta didik mampu memahami konsep 
tentang gerak rotasi dengan kecepatan sudut konstan serta mampu mengamati dan mengidentifikasi benda di sekitar 
yang mengalami gerak tersebut. Kemudian, peserta didik mampu memperdalam pemahaman fisika sesuai dengan minat untuk 
melanjutkan ke perguruan tinggi yang berhubungan dengan bidang fisika. Melalui kerja ilmiah, juga dibangun sikap ilmiah 
dan Profil Pelajar Pancasila, khususnya mandiri, inovatif, bernalar kritis, kreatif, dan bergotong royong.
        ...
        st.markdown("""
1. Peserta didik mampu menjelaskan konsep dinamika rotasi melalui eksplorasi langsung pada aplikasi simulasi berbasis Streamlit.  
2. Peserta didik mampu menerapkan prinsip dinamika rotasi untuk memecahkan masalah kontekstual melalui simulasi dan latihan interaktif di platform Streamlit.  
3. Peserta didik mampu menganalisis hubungan antara momen gaya, momen inersia, dan percepatan sudut dengan mengubah nilai parameter dalam simulasi virtual berbasis Streamlit.
        """)
    ...
    # --- MENU: Simulasi Virtual ---
        ...
        check_hadir()
        st.info("Simulasi virtual akan ditampilkan di sini. Anda bisa mengaksesnya melalui tautan atau iframe.")

    # --- MENU: LKPD ---
    elif menu == "LKPD":
        st.subheader("📄 LKPD: Dinamika Rotasi")
        check_hadir()
        lkpd_data = muat_data(LKPD_INFO_FILE)
        if not lkpd_data:
        ...
        # Tampilkan Tujuan, Dasar Teori, dll.
        st.markdown("### 🎯 Tujuan Pembelajaran")
        ...
        if st.button("✅ Simpan Jawaban LKPD"):
            if not nama.strip() or not kelompok.strip():
                ...
                df = pd.concat([df, new_entry], ignore_index=True)
                simpan_data(HASIL_NILAI_FILE, df)
    ...
    # --- MENU: Refleksi Siswa ---
        ...
        for i, q in enumerate(refleksi_data.get("pertanyaan_list", []), 1):
            ...
            jawaban_key = q["id"]
            jawaban = st.text_area("Jawaban Anda:", key=jawaban_key, height=100)
            jawaban_refleksi[jawaban_key] = jawaban
    ...
    # --- MENU: Post-test ---
        ...
        st.subheader("📝 Post-test: Dinamika Rotasi")
        check_hadir()
        post_test_data = muat_data(POST_TEST_INFO_FILE)
        if not post_test_data:
        ...
        if st.button("✅ Kirim Jawaban Post-test"):
            if any(not v.strip() for v in jawaban_dict.values()):
                ...
                # Simpan jawaban & nilai ke file CSV
                df = muat_data(HASIL_NILAI_FILE)
                jawaban_json_str = json.dumps(jawaban_dict)
                new_entry = pd.DataFrame([{
                    "email": st.session_state.current_email,
